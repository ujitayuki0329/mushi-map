...
if (!firebaseConfig.apiKey || !firebaseConfig.projectId) {
  console.error('Firebase設定が不足しています。.envファイルを確認してください。');
  process.exit(1);
}
...
const insectNames = [
  ...
  'オオゴマダラ', 'アカエリトリバネアゲハ', 'オオゴマダラ', 'アオバセセリ', 'イチモンジセセリ',
  'オオスズメバチ', 'キイロスズメバチ', 'クロスズメバチ', 'セグロアシナガバチ', 'フタモンアシナガバチ',
  'オオカマキリ', 'ハラビロカマキリ', 'チョウセンカマキリ', 'コカマキリ', 'ウスバカマキリ',
  'ショウリョウバッタ', 'オンブバッタ', 'クルマバッタ', 'トノサマバッタ', 'イナゴ',
  'コオロギ', 'エンマコオロギ', 'マツムシ', 'スズムシ', 'カネタタキ',
  'アブラゼミ', 'ミンミンゼミ', 'ツクツクボウシ', 'ヒグラシ', 'ニイニイゼミ',
  'オオシオカラトンボ', 'シオカラトンボ', 'アキアカネ', 'ナツアカネ', 'コノハムシ',
  'ナナフシ', 'カマキリ', 'ハナカマキリ', 'コカマキリ', 'ウスバカマキリ'
];
...
const generateTestData = async () => {
  ...
  for (let i = 0; i < 100; i++) {
    try {
      ...
      await addDoc(collection(db, 'insectEntries'), entryData);
